...
While State Space Models (SSMs), particularly Mamba, have demonstrated significant potential in sequence modeling with linear complexity, their application to 3D point cloud analysis remains challenging due to the inherent disparity between 1D sequence structures and unordered 3D geometry. Existing methods often rely on simple serialization strategies that distort spatial adjacency, leading to suboptimal feature learning and a loss of long-sequence memory. To fully unleash the potential of Mamba for point clouds, we propose ElaMamba, a novel framework that implicitly preserves structural dependencies within the state space itself. Our approach moves beyond static serialization by introducing a differentiable Spatial Micro-Deformation Generation mechanism, which adaptively searches for relevant geometric structures. To regularize this process, we propose a novel Adaptive Regularization Loss based on the $L_{2}$ norm of the predicted offsets, forcing the network to degenerate into a rigid scan on clean data while allowing necessary deformations to bypass noise in real-world scenarios. Furthermore, to prevent the assimilation of irrelevant background noise during deformation, we integrate an Offset-Aware Dynamic Gating Mechanism that selectively modulates feature propagation. Extensive experiments demonstrate that ElaMamba effectively adapts to varying sequence lengths and achieves state-of-the-art performance, reaching 92.99\% overall accuracy on ModelNet40 and 88.34\% on the challenging ScanObjectNN dataset, significantly outperforming existing Transformer and Mamba-based architectures.
