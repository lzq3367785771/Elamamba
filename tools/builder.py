...
def load_model(base_model, ckpt_path, logger=None):
    ...
    # load state dict
    state_dict = torch.load(ckpt_path, map_location='cpu')
    # --- 修改的核心部分：增加 .replace("dm3d_scan", "ess_module") ---
    if state_dict.get('model') is not None:
        # 将权重中的 dm3d_scan 映射为代码中的 ess_module
        base_ckpt = {k.replace("module.", "").replace("dm3d_scan", "ess_module"): v for k, v in state_dict['model'].items()}
    elif state_dict.get('base_model') is not None:
        base_ckpt = {k.replace("module.", "").replace("dm3d_scan", "ess_module"): v for k, v in state_dict['base_model'].items()}
    else:
