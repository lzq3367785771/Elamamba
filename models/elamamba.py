...
class ElaMamba(nn.Module):
    def __init__(self, config, **kwargs):
        super(ElaMamba, self).__init__()
        self.config = config
        ...
        self.order_indicator_1 = SpatialDynamicIndicator(self.trans_dim)
        self.order_indicator_2 = SpatialDynamicIndicator(self.trans_dim)

        self.ess_module = ElasticStructuralScanning(self.trans_dim)
    ...
    def load_model_from_ckpt(self, bert_ckpt_path):
        if bert_ckpt_path is not None:
            ...
            if incompatible.missing_keys:
                print_log('missing_keys', logger='ElaMamba')
                print_log(
                    get_missing_parameters_message(incompatible.missing_keys),
                    logger='ElaMamba'
                )
            if incompatible.unexpected_keys:
                print_log('unexpected_keys', logger='ElaMamba')
                print_log(
                    get_unexpected_parameters_message(incompatible.unexpected_keys),
                    logger='ElaMamba'
                )

            print_log(f'[ElaMamba] Successful Loading the ckpt from {bert_ckpt_path}', logger='ElaMamba')
        else:
            print_log('Training from scratch!!!', logger='ElaMamba')
    ...
    def forward(self, pts):
        ...
        center_fwd, _, _, tokens_forward, _ = serialization_func(center, group_input_tokens, pos, 'hilbert')
        center_bwd, _, _, tokens_backward, _ = serialization_func(center, group_input_tokens, pos, 'hilbert-trans')

        dynamic_center_fwd, tokens_forward, offset_fwd = self.ess_module(center_fwd, tokens_forward)
        dynamic_center_bwd, tokens_backward, offset_bwd = self.ess_module(center_bwd, tokens_backward)

        offsets = torch.cat([offset_fwd, offset_bwd], dim=1)

        dynamic_pos_fwd = self.pos_embed(dynamic_center_fwd)
        dynamic_pos_bwd = self.pos_embed(dynamic_center_bwd)

        tokens_forward = apply_OrderScale(tokens_forward, self.OrderScale_gamma_1, self.OrderScale_beta_1)
        tokens_backward = apply_OrderScale(tokens_backward, self.OrderScale_gamma_2, self.OrderScale_beta_2)

        tokens_forward = self.order_indicator_1(tokens_forward, dynamic_pos_fwd)
        tokens_backward = self.order_indicator_2(tokens_backward, dynamic_pos_bwd)

        if self.use_cls_token:
            cls_token = self.cls_token.expand(tokens_forward.size(0), -1, -1)
            cls_pos = self.cls_pos.expand(tokens_forward.size(0), -1, -1)
            pos = torch.cat([dynamic_pos_fwd, dynamic_pos_bwd, cls_pos], dim=1)
            group_input_tokens = torch.cat([tokens_forward, tokens_backward, cls_token], dim=1)
        else:
            pos = torch.cat([dynamic_pos_fwd, dynamic_pos_bwd], dim=1)
            group_input_tokens = torch.cat([tokens_forward, tokens_backward], dim=1)
        ...
        x_f, x_b = torch.chunk(x_tokens, 2, dim=1)
        
        feat_f = x_f.mean(dim=1) + x_f.max(dim=1)[0]
        feat_b = x_b.mean(dim=1) + x_b.max(dim=1)[0]
        
        concat_f = (feat_f + feat_b) / 2.0

        if cls_feat is not None:
            concat_f = concat_f + cls_feat

        ret = self.cls_head_finetune(concat_f)

        if self.training:
            return ret, concat_f, offsets
        else:
            return ret, concat_f
...
class MaskMamba(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        print_log(f'[args] {config.mamba_config}', logger='ElaMamba')
...
class MambaDecoder(nn.Module):
    ...
    def forward(self, x, pos, mask_pos):
        B, _, C = x.shape
        x = self.blocks(x, pos)
        x = self.head(x[mask_pos, :].view(B, -1, C))
        return x


@MODELS.register_module()
class ElaMamba_MAE_serialization(nn.Module):
    def __init__(self, config):
        super().__init__()
        print_log(f'[ElaMamba] ', logger='ElaMamba')
        self.config = config
        self.trans_dim = config.mamba_config.trans_dim
        ...
        print_log(f'[ElaMamba] divide point cloud into G{self.num_group} x S{self.group_size} points ...',
                  logger='ElaMamba')
        self.group_divider = Group(num_group=self.num_group, group_size=self.group_size)
...
class SpatialDynamicIndicator(nn.Module):
    ...
    def forward(self, x, pos):
        feat_pos = torch.cat([x, pos], dim=-1)

        stats = self.fc(feat_pos)
        gamma, beta = torch.chunk(stats, 2, dim=-1)

        return x * (1 + gamma) + beta


class ElasticStructuralScanning(nn.Module):
    def __init__(self, dim):
        super().__init__()
    ...
    def forward(self, base_pos, features):
        offsets = torch.tanh(self.offset_predictor(features)) * 0.1

        dynamic_pos = base_pos + offsets

        feat_with_offsets = torch.cat([features, offsets], dim=-1)
        gate_weights = self.dynamic_gate(feat_with_offsets)

        modulated_features = features * gate_weights

        return dynamic_pos, modulated_features, offsets
